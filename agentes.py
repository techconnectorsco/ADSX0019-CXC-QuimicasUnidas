...
TRADUCCION_TIPOS = {
    ...
    'NC': 'NC', 'NCM': 'NCM', 'N/C': 'N/C', 'RC': 'RC',
    'REM': 'REM', 'PR': 'PR', 'ND': 'ND', 'NDM': 'NDM',
    'N/D': 'N/D', 'AS': 'AS',
}
...
def obtener_todos_paginado(conn: ServiceLayerConnection, entidad: str, params: dict, campo_orden: str = "DocNum") -> List[Dict]:
    todos = []
    skip = 0
    page_size = 20
...
def obtener_clientes_con_saldo(conn: ServiceLayerConnection, limite: int = None) -> List[Dict]:
    params = {
...
def obtener_vendedores(conn: ServiceLayerConnection) -> Dict[int, Dict]:
    ...
    resultado = obtener_todos_paginado(conn, "SalesPersons", params, "SalesEmployeeCode")
...
def obtener_contacto_principal(conn: ServiceLayerConnection, card_code: str, contact_code: int) -> Dict:
    if not contact_code:
        return {'nombre': '', 'telefono': '', 'email': ''}
    try:
        bp = conn.get(f"BusinessPartners('{card_code}')", {"$select": "ContactEmployees"})
            ...
            for contacto in bp['ContactEmployees']:
                if contacto.get('InternalCode') == contact_code:
                    return {
                        ...
                        'telefono': contacto.get('Phone1', '') or contacto.get('MobilePhone', ''),
...
def obtener_documentos_cliente(conn: ServiceLayerConnection, card_code: str) -> List[Dict]:
    documentos = []
    
    facturas = obtener_todos_paginado(conn, "Invoices", {
    ...
    for f in facturas:
        ...
        if doc: documentos.append(doc)
            
    notas_credito = obtener_todos_paginado(conn, "CreditNotes", {
        "$filter": f"CardCode eq '{card_code}' and DocumentStatus eq 'bost_Open'",
        "$select": "DocNum,DocEntry,DocDate,DocDueDate,DocTotal,DocTotalFc,PaidToDate,PaidToDateFC,DocCurrency,U_TDOC,U_NVT_ConsecutivoFE,U_NUM_CONSE,NumAtCard,Comments,DocumentLines",
    }, "DocDueDate")
...
def procesar_datos_cliente(conn: ServiceLayerConnection, cliente: Dict) -> Optional[Dict]:
...
def ejecutar_reportes_gira():
    ...
    try:
        ...
        clientes = obtener_clientes_con_saldo(conn, limite=20 if '--test' in sys.argv else None)
        ...
        for vendedor_id, clientes_del_agente in agrupados_por_agente.items():
            ...
            clientes_del_agente.sort(key=lambda c: (
            ...
            datos_reporte['agente']['zonas'] = ", ".join(zonas_list) if zonas_list else "Múltiples/No Definida"
            
            resultados['procesados'] += 1
            
            print(f"   ✅ Clientes con documentos: {len(datos_reporte['clientes'])}")
            print(f"   💰 Total a cobrar USD: ${datos_reporte['totales_agente']['dolares']:,.2f}")
            print(f"   💰 Total a cobrar CRC: ₡{datos_reporte['totales_agente']['colones']:,.2f}")
