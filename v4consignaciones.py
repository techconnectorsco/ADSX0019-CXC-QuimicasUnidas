...
def odata_paginado(conn, entidad, params=None, page_size=500):
    """
    Trae todos los registros de un endpoint OData paginando con páginas grandes.

    Usa header 'Prefer: odata.maxpagesize=N' para reducir round-trips HTTP.
    Si SAP no respeta el header, igualmente paginamos correctamente (de 20 en 20).
    """
    if params is None:
        params = {}
    todos = []
...
def obtener_series_de_bodegas(conn, codigos_bodega):
    """
    Trae las series en stock con UNA SOLA consulta + páginas grandes.

    APRENDIZAJE: lo lento no es HANA, son los round-trips HTTP al Service Layer
    en la nube (~700ms cada uno). La solución es minimizar requests:
      - 1 sola query con IN (...) para todas las bodegas
      - Header 'Prefer: odata.maxpagesize=500' para traer 500 por página
        en vez de 20 (default)

    Esto reduce 250 requests HTTP a ~10.
    """
    print(
        f"📥 Series en {len(codigos_bodega)} bodegas (consulta única + páginas grandes)..."
    )
    t0 = time.time()

    in_list = ",".join(f"'{c}'" for c in codigos_bodega)
    # CRÍTICO: usamos OSRI (no OSRQ) con Status=0 para traer SOLO series
    # actualmente disponibles. OSRQ tiene histórico que infla los resultados.
    # IntrSerial es el "Serial interno" mostrado en SAP. Si la encargada quiere
    # ver el "Mfr Serial" (número que viene del proveedor), cambiar a SuppSerial.
    sql = (
        'SELECT T0."WhsCode", T0."ItemCode", '
        'T0."IntrSerial" AS "SerialNumber", '
        'T0."SuppSerial" AS "SuppSerial" '
        "FROM OSRI T0 "
        f'WHERE T0."Status"=0 AND T0."WhsCode" IN ({in_list})'
    )
    ...
    try:
        print(f"   ⏳ Creando query temporal...")
        resp = conn.session.post(
            url_post, json={"SqlCode": query_code, "SqlName": "TMP", "SqlText": sql}
        )
        if resp.status_code not in (200, 201):
            ...
            try:
                ...
                pass
            return []
        print(f"   ⏳ Ejecutando con páginas de 500...")

        # Headers para pedir páginas grandes
        headers_grande = {"Prefer": "odata.maxpagesize=500"}

        skip = 0
        pagina = 0
        while True:
            pagina += 1
            url = f"{conn.base_url}/SQLQueries('{query_code}')/List"
            res_http = conn.session.get(
                url, params={"$skip": skip}, headers=headers_grande
            )

            if res_http.status_code != 200:
                print(f"   ❌ Error página {pagina}: {res_http.status_code}")
                break
            ...
            print(
                f"   Página {pagina}: +{len(res['value']):4} (total: {len(series):5}) | {time.time()-t0:.1f}s"
            )

            # Si SAP no respetó el maxpagesize y devolvió 20, igual avanzamos correctamente
            recibidos = len(res["value"])
            if recibidos < 20:
                break
            skip += recibidos
    ...
    print(f"   ✅ {len(series)} series en {time.time()-t0:.1f}s")
    return series
