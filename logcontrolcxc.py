...
def generar_qr_log(stats: Dict, fecha: str, hora: str) -> str:
    ...
    contenido = []
    contenido.append("==============================")
    contenido.append("   QUIMICAS UNIDAS S.A.")
    ...
    contenido.append("")
    contenido.append(f"Cartera USD: ${formato_latino(stats.get('total_cartera_usd', 0))}")
    contenido.append(f"Cartera CRC: {formato_latino(stats.get('total_cartera_crc', 0))}")
    contenido.append("")
    contenido.append("==============================")
...
class LogControlCXC(FPDF):
    ...
    def header(self):
        ...
        self.set_y(12)
        self.cell(0, 8, 'Quimicas Unidas S.A.', 0, 1, 'C')
    ...
    def footer(self):
        ...
        self.set_y(-18)
        self.cell(0, 5, 'Departamento de Credito y Cobros | Tel: 2257-8484 ext. 216-217', 0, 1, 'C')
    ...
    def agregar_resumen_ejecutivo(self, stats: Dict):
        ...
        self.cell(60, 6, f"Clientes consultados: {stats.get('total_clientes', 0)}", 0, 0)
        ...
        # Deshabilitados
        self.set_text_color(*GRIS)
        self.cell(60, 6, f"[DES] Envio deshabilitado: {stats.get('deshabilitados', 0)}", 0, 1)
        ...
        self.set_text_color(*ROJO)
        self.cell(80, 6, f"Vencido: {pct_vencido_usd:.1f}% USD | {pct_vencido_crc:.1f}% CRC", 0, 1)
    ...
    def agregar_tabla_detalle(self, registros: List[Dict]):
        ...
        registros_con_docs = [r for r in registros if r.get('docs_usd', 0) > 0 or r.get('docs_crc', 0) > 0]
        ...
        headers = ['Codigo', 'Cliente', 'Correo(s)', 'Docs USD', 'Docs CRC', 'Total USD', 'Total CRC', 'PDF', 'Email', 'Observacion']
        ...
        for reg in registros_con_docs:
            ...
            self.cell(anchos[5], 5, f"${formato_latino(total_usd)}" if total_usd != 0 else '-', 1, 0, 'R', True)
            
            # Total CRC
            total_crc = reg.get('total_crc', 0)
            self.cell(anchos[6], 5, f"CRC {formato_latino(total_crc)}" if total_crc != 0 else '-', 1, 0, 'R', True)
    ...
    def agregar_top_saldos(self, top_clientes: List[Dict]):
        ...
        for idx, cliente in enumerate(top_clientes[:10], 1):
            ...
            self.cell(anchos[2], 6, f"${formato_latino(cliente.get('total_usd', 0))}", 1, 0, 'R', True)
            self.cell(anchos[3], 6, f"CRC {formato_latino(cliente.get('total_crc', 0))}", 1, 1, 'R', True)
...
class ControlCXC:
    ...
    def finalizar(self):
        ...
        for reg in self.registros:
            reg['saldo_total'] = reg['total_usd'] + (reg['total_crc'] / 500)  # Conversión aproximada
