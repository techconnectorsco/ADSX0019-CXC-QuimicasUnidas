...
def generar_qr_validacion(datos_cliente: Dict, docs_usd: List, docs_crc: List, totales: Dict) -> str:
    """
...
class PDFEstadoCuenta(FPDF):
    ...
    def footer(self):
        ...
        self.set_y(-18)
        self.cell(0, 5, 'Departamento de Crédito y Cobros | Tel: 2257-8484 ext. 216-217', 0, 1, 'C')
    ...
    def agregar_tabla_unica(self, docs_usd: List[Dict], docs_crc: List[Dict], totales: Dict):
        todos_docs = []
        ...
        if docs_usd:
            ...
            self.cell(anchos[-2] + anchos[-1], 6, f"USD {formato_latino(totales['dolares'])}", 1, 1, 'R', True)
            
        if docs_crc:
            self.cell(ancho_blanco, 6, 'TOTAL GENERAL COLONES:', 1, 0, 'R', True)
            self.cell(anchos[-2] + anchos[-1], 6, f"CRC {formato_latino(totales['colones'])}", 1, 1, 'R', True)
    ...
    def agregar_seccion_vencidos_unificada(self, rangos_usd: Dict, rangos_crc: Dict):
        ...
        def dibujar_tabla_individual(x_pos, rangos, moneda_texto, prefijo, total_v):
            ...
            self.ln(2)
            labels = [('Total 0-30', '0_30'), ('Total 31-60', '31_60'), 
                      ('Total 61-90', '61_90'), ('Total 91-120', '91_120'), 
                      ('Total 120+', 'mas_120')]
            ...
            self.cell(w_tabla * 0.5, 7, f"{prefijo} {formato_latino(total_v)}", 1, 1, 'R', True)
        ...
        if dibujar_usd:
            final_y_usd = dibujar_tabla_individual(x_inicio, rangos_usd, "Dolares", "USD", vencido_usd)
        
        if dibujar_crc:
            x_pos_crc = (x_inicio + w_tabla + gap) if dibujar_usd else x_inicio
            final_y_crc = dibujar_tabla_individual(x_pos_crc, rangos_crc, "Colones", "CRC", vencido_crc)
